...
function(Y,penalty="fused",lambda1,lambda2,rho=1,weights="equal",penalize.diagonal=FALSE,maxiter=500,tol=1e-5,warm=NULL,return.whole.theta=FALSE, screening="fast",truncate=1e-5)
{
	...
	if(is.list(lambda1)){
		lam1 = lapply(lambda1, function(x) x[connected,connected])
	}
	else{
		lam1 = lambda1
	...
	if(penalty=="fused")
	{
  	...
  	crit2 = list()
	if(is.list(lam1)){
		for(k in 1:K) { crit1[[k]] =  abs(S[[k]])*weights[k] > lam1[[k]] + lam2 }  
	}
	else{
  	...
  	S.sum = abs(S.sum)
	if(is.list(lam1)){
		for(k in 1:K) { crit2[[k]] = S.sum > 2*lam1[[k]] }
	}
	else{
		for(k in 1:K) { crit2[[k]] = S.sum > 2*lam1 }
	}
  	}
  	...
  	crit1 = list()
	if(is.list(lam1)){
		for(k in 1:K) { crit1[[k]] =  abs(S[[k]])*weights[k] > lam1[[k]] }  
	}
	else{
	...
	if(is.list(lam1)){
		critboth = 0
		for(k in 1:K) { critboth = critboth + crit2[[k]]}
	}
	else{
		critboth = crit2[[1]]
	}
	...
	if(is.list(lam1)){
		for(k in 1:K) {tempsum = tempsum + (pmax(weights[k]*abs(S[[k]]) - lam1[[k]],0))^2 }    
	}
	else{
...
#	}
	...
	cat('Getting solution for unconnected nodes\n')
	# data:
	cat('Define data\n')
	Yu = list()
	...
	cat('Penalty vectors\n')
	if(is.list(lambda1)){
		lam1.unconnected = list()
		for(i in 1:length(lambda1)){ lam1.unconnected[[i]] = diag(lambda1[[i]])[unconnected] }
	}
	else{
	...
	cat('Penalized diagonal\n')
	if(!penalize.diagonal)
	{ 
		if(is.list(lambda1)){
			lam1.unconnected = lapply(lam1.unconnected, function(x) x*0)
		}
		else{ lam1.unconnected = lam1.unconnected * 0 }
	...
	for(i in 1:length(blocklist)){
		...
		if(is.list(lambda1)){
			lam1.bl = list()
			for(i in 1:length(lambda1)){lam1.bl[[i]] = penalty.as.matrix(lambda1[[i]][bl,bl],dim(Ybl[[1]])[2],penalize.diagonal=penalize.diagonal)}
		}
		else{
