...
patients_controls, genenames_cases, Mexp_cases = ett.io.read_expression_matrix(args.cases_expression)
patients_controls, genenames_controls, Mexp_controls = ett.io.read_expression_matrix(args.controls_expression)
...
conf_val_cases = ett.io.read_dict(args.cases_bootstrap, numfieldsid=2)
conf_val_controls = ett.io.read_dict(args.controls_bootstrap, numfieldsid=2, apply_for_keys=lambda x: (int(x[0]), int(x[1])), apply_for_values=lambda x:int(x[0]))
nodesbycluster, edgesbycluster = ett.io.read_community(args.cases_communities)
...
ett.io.write_dict(args.outfile, traits, split_lists=True)
